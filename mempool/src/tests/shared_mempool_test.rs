...
use aptos_types::{transaction::{ReplayProtector, Transaction}, vm_status::DiscardedVMStatus};
...
async fn test_consensus_events_rejected_txns() {
    ...
    let rejected_txn = TestTransaction::new(0,ReplayProtector::SequenceNumber(0), 1).make_signed_transaction();
    let kept_txn_1 = TestTransaction::new(1, ReplayProtector::SequenceNumber(0), 1).make_signed_transaction();
    let kept_txn_2 = TestTransaction::new(0, ReplayProtector::SequenceNumber(1), 1).make_signed_transaction();
...
async fn test_mempool_notify_committed_txns() {
    ...
    let committed_txn =
        TestTransaction::new(0, ReplayProtector::SequenceNumber(0), 1).make_signed_transaction_with_expiration_time(0);
    let kept_txn = TestTransaction::new(1, ReplayProtector::SequenceNumber(0), 1).make_signed_transaction(); // not committed or cleaned out by block timestamp gc
    let txns = vec![
        committed_txn.clone(),
        TestTransaction::new(0, ReplayProtector::SequenceNumber(1), 1).make_signed_transaction_with_expiration_time(0),
        kept_txn.clone(),
    ];
