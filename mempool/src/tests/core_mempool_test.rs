...
use crate::{
    core_mempool::{sender_bucket, AccountSequenceNumberInfo, CoreMempool, MempoolTransaction, SubmittedBy, TimelineState},
...
fn test_transaction_metrics() {
    ...
    let txn = TestTransaction::new(0, ReplayProtector::SequenceNumber(0), 1).make_signed_transaction();
    mempool.add_txn(
        txn.clone(),
        txn.gas_unit_price(),
        AccountSequenceNumberInfo::Required(0),
        TimelineState::NotReady,
        false,
        None,
        Some(BroadcastPeerPriority::Primary),
    );
    let txn = TestTransaction::new(1, ReplayProtector::SequenceNumber(0), 1).make_signed_transaction();
    mempool.add_txn(
        txn.clone(),
        txn.gas_unit_price(),
        AccountSequenceNumberInfo::Required(0),
    ...
    let txn = TestTransaction::new(2, ReplayProtector::SequenceNumber(0), 1).make_signed_transaction();
    mempool.add_txn(
        txn.clone(),
        txn.gas_unit_price(),
        AccountSequenceNumberInfo::Required(0),
    ...
    let (insertion_info, _bucket, _priority) = mempool
        ...
        .get_insertion_info_and_bucket(&TestTransaction::get_address(0), ReplayProtector::SequenceNumber(0))
        .unwrap();
    ...
    let (insertion_info, _bucket, _priority) = mempool
        .get_transaction_store()
        .get_insertion_info_and_bucket(&TestTransaction::get_address(1), ReplayProtector::SequenceNumber(0))
        .unwrap();
    assert_eq!(insertion_info.submitted_by, SubmittedBy::PeerValidator);

    let (insertion_info, _bucket, _priority) = mempool
        .get_transaction_store()
        .get_insertion_info_and_bucket(&TestTransaction::get_address(2), ReplayProtector::SequenceNumber(0))
        .unwrap();
...
fn test_update_transaction_in_mempool() {
    ...
    let fixed_txns = add_txns_to_mempool(&mut mempool, vec![TestTransaction::new(0, ReplayProtector::SequenceNumber(0), 5)]);
...
fn test_ignore_same_transaction_submitted_to_mempool() {
    let (mut mempool, _) = setup_mempool();
    let _ = add_txns_to_mempool(&mut mempool, vec![TestTransaction::new(0, ReplayProtector::SequenceNumber(0), 0)]);
    let ret = add_txn(&mut mempool, TestTransaction::new(0, ReplayProtector::SequenceNumber(0), 0));
    assert!(ret.is_ok())
}
...
fn test_reject_transaction() {
    ...
    assert!(pool
        .get_transaction_store()
        .get(&TestTransaction::get_address(0), ReplayProtector::SequenceNumber(0))
        .is_some());
    pool.reject_transaction(
        &TestTransaction::get_address(0),
    ...
    assert!(pool
        .get_transaction_store()
        .get(&TestTransaction::get_address(0), ReplayProtector::SequenceNumber(1))
        .is_some());
...
fn test_system_ttl() {
    ...
    add_txn(&mut mempool, TestTransaction::new(0, ReplayProtector::SequenceNumber(0), 10)).unwrap();
...
fn test_commit_callback() {
    ...
    let txns = add_txns_to_mempool(&mut pool, vec![TestTransaction::new(1, ReplayProtector::SequenceNumber(6), 1)]);

    // Check that pool is empty.
    assert!(pool.get_batch(1, 1024, true, btreemap![]).is_empty());
    // Transaction 5 got back from consensus.
    pool.commit_transaction(&TestTransaction::get_address(1), ReplayProtector::SequenceNumber(5));
...
fn test_reset_sequence_number_on_failure() {
    let mut pool = setup_mempool().0;
    let txns = [TestTransaction::new(1, ReplayProtector::SequenceNumber(0), 1), TestTransaction::new(1, ReplayProtector::SequenceNumber(1), 1)];
    ...
    assert!(add_txn(&mut pool, TestTransaction::new(1, ReplayProtector::SequenceNumber(0), 1)).is_ok());
}
...
fn test_timeline() {
    ...
    add_txns_to_mempool(&mut pool, vec![TestTransaction::new(1, ReplayProtector::SequenceNumber(2), 1)]);
    let (timeline, _) = pool.read_timeline(
        sender_bucket,
        &vec![0].into(),
        10,
    ...
    pool.commit_transaction(&TestTransaction::get_address(1), ReplayProtector::SequenceNumber(4));
    let (timeline, _) = pool.read_timeline(
        sender_bucket,
        &vec![0].into(),
        10,
...
fn test_multi_bucket_gas_ranking_update() {
    ...
    add_txns_to_mempool(&mut pool, vec![TestTransaction::new(1, ReplayProtector::SequenceNumber(2), 400)]);
...
fn test_multi_bucket_removal() {
    ...
    pool.commit_transaction(&TestTransaction::get_address(1), ReplayProtector::SequenceNumber(0));
    let (timeline, _) = pool.read_timeline(
        sender_bucket,
        &vec![0, 0, 0].into(),
        10,
    ...
    pool.commit_transaction(&TestTransaction::get_address(1), ReplayProtector::SequenceNumber(1));
    let (timeline, _) = pool.read_timeline(
        sender_bucket,
        &vec![0, 0, 0].into(),
        10,
    ...
    pool.commit_transaction(&TestTransaction::get_address(1), ReplayProtector::SequenceNumber(2));
    let (timeline, _) = pool.read_timeline(
        sender_bucket,
        &vec![0, 0, 0].into(),
        10,
    ...
    pool.commit_transaction(&TestTransaction::get_address(1), ReplayProtector::SequenceNumber(3));
    let (timeline, _) = pool.read_timeline(
        sender_bucket,
        &vec![0, 0, 0].into(),
        10,
...
fn test_capacity() {
    ...
    add_txn(&mut pool, TestTransaction::new(1, ReplayProtector::SequenceNumber(0), 1)).unwrap();
    assert!(add_txn(&mut pool, TestTransaction::new(1, ReplayProtector::SequenceNumber(1), 1)).is_err());

    // Commit transaction and free space.
    pool.commit_transaction(&TestTransaction::get_address(1), ReplayProtector::SequenceNumber(0));
    assert!(add_txn(&mut pool, TestTransaction::new(1, ReplayProtector::SequenceNumber(1), 1)).is_ok());

    // Fill it up and check that GC routine will clear space.
    assert!(add_txn(&mut pool, TestTransaction::new(1, ReplayProtector::SequenceNumber(2), 1)).is_err());
    pool.gc();
    assert!(add_txn(&mut pool, TestTransaction::new(1, ReplayProtector::SequenceNumber(2), 1)).is_ok());
}
...
fn test_capacity_bytes() {
    ...
    loop {
        let txn = signed_txn_to_mempool_transaction(
            TestTransaction::new(address, ReplayProtector::SequenceNumber(seq_no), 1).make_signed_transaction(),
        );
...
fn test_parking_lot_eviction() {
    ...
    for seq in &[0, 1, 2, 9, 10] {
        add_txn(&mut pool, TestTransaction::new(1, ReplayProtector::SequenceNumber(*seq), 1)).unwrap();
    }
    // Mempool is full. Insert few txns for other account.
    for seq in &[0, 1] {
        add_txn(&mut pool, TestTransaction::new(0, ReplayProtector::SequenceNumber(*seq), 1)).unwrap();
    }
    ...
    assert!(add_txn(&mut pool, TestTransaction::new(0, ReplayProtector::SequenceNumber(2), 1)).is_err());
}
...
fn test_parking_lot_eviction_bytes() {
    ...
    let small_txn_size =
        signed_txn_to_mempool_transaction(TestTransaction::new(1, ReplayProtector::SequenceNumber(1), 1).make_signed_transaction())
            .get_estimated_bytes();
    ...
    for address in 0..2 {
        add_txn(&mut pool, TestTransaction::new(address, ReplayProtector::SequenceNumber(1), 1)).unwrap();
    }
    // Add one large transaction that will top off the capacity bytes
    add_txn(&mut pool, TestTransaction::new_with_large_script(2, ReplayProtector::SequenceNumber(1), 1)).unwrap();
    // Mempool is full. Insert a small txn for other account.
    add_txn(&mut pool, TestTransaction::new(3, ReplayProtector::SequenceNumber(0), 1)).unwrap();
}
...
fn test_parking_lot_eviction_benchmark() {
    ...
    let huge_txn_size = signed_txn_to_mempool_transaction(
        TestTransaction::new_with_huge_script(1, ReplayProtector::SequenceNumber(1), 1).make_signed_transaction(),
    )
    .get_estimated_bytes();
    ...
    for account in accounts {
        for seq_num in 1..2 {
            add_txn(
                &mut pool,
                TestTransaction::new_with_address(account, ReplayProtector::SequenceNumber(seq_num), 1),
            )
            .unwrap();
        }
    }
    // Add one huge transaction that will cause mempool to be (beyond) full
    let huge_signed_txn = TestTransaction::new_with_huge_script(0, ReplayProtector::SequenceNumber(0), 1).make_signed_transaction();
    ...
    let huge_signed_txn = TestTransaction::new_with_huge_script(1, ReplayProtector::SequenceNumber(0), 1).make_signed_transaction();
    // Pre-compute these values, as shared mempool would do
    huge_signed_txn.committed_hash();
    huge_signed_txn.txn_bytes_len();
    let now = Instant::now();
...
fn test_parking_lot_evict_only_for_ready_txn_insertion() {
    ...
    for seq in not_ready_seq_nums {
        assert!(add_txn(&mut pool, TestTransaction::new(1, ReplayProtector::SequenceNumber(seq), 1)).is_err());
    }
}
...
fn test_gc_ready_transaction() {
    ...
    let txn = TestTransaction::new(1, ReplayProtector::SequenceNumber(1), 1).make_signed_transaction_with_expiration_time(0);
    ...
    add_txn(&mut pool, TestTransaction::new(1, ReplayProtector::SequenceNumber(2), 1)).unwrap();
    add_txn(&mut pool, TestTransaction::new(1, ReplayProtector::SequenceNumber(3), 1)).unwrap();
    ...
    // Resubmit txn 1
    add_txn(&mut pool, TestTransaction::new(1, ReplayProtector::SequenceNumber(1), 1)).unwrap();
...
fn test_clean_stuck_transactions() {
    ...
    let txn = TestTransaction::new(0, ReplayProtector::SequenceNumber(db_sequence_number), 1).make_signed_transaction();
    pool.add_txn(
        txn,
        1,
        AccountSequenceNumberInfo::Required(db_sequence_number),
        TimelineState::NotReady,
        false,
        None,
        Some(BroadcastPeerPriority::Primary),
...
fn test_get_transaction_by_hash_after_the_txn_is_updated() {
    ...
    let new_txn = TestTransaction::new(0, ReplayProtector::SequenceNumber(db_sequence_number), 100).make_signed_transaction();
    pool.add_txn(
        new_txn.clone(),
        1,
        AccountSequenceNumberInfo::Required(db_sequence_number),
...
fn test_transaction_store_remove_account_if_empty() {
    ...
    add_txn(&mut pool, TestTransaction::new(1, ReplayProtector::SequenceNumber(0), 1)).unwrap();
    add_txn(&mut pool, TestTransaction::new(1, ReplayProtector::SequenceNumber(1), 1)).unwrap();
    add_txn(&mut pool, TestTransaction::new(2, ReplayProtector::SequenceNumber(0), 1)).unwrap();
    assert_eq!(pool.get_transaction_store().get_transactions().len(), 2);

    pool.commit_transaction(&TestTransaction::get_address(1), ReplayProtector::SequenceNumber(0));
    pool.commit_transaction(&TestTransaction::get_address(1), ReplayProtector::SequenceNumber(1));
    pool.commit_transaction(&TestTransaction::get_address(2), ReplayProtector::SequenceNumber(0));
    assert_eq!(pool.get_transaction_store().get_transactions().len(), 0);

    let txn = TestTransaction::new(2, ReplayProtector::SequenceNumber(2), 1).make_signed_transaction();
    let hash = txn.committed_hash();
...
fn test_sequence_number_behavior_at_capacity() {
    ...
    add_txn(&mut pool, TestTransaction::new(0, ReplayProtector::SequenceNumber(0), 1)).unwrap();
    add_txn(&mut pool, TestTransaction::new(1, ReplayProtector::SequenceNumber(0), 1)).unwrap();
    pool.commit_transaction(&TestTransaction::get_address(1), ReplayProtector::SequenceNumber(0));
    add_txn(&mut pool, TestTransaction::new(2, ReplayProtector::SequenceNumber(0), 1)).unwrap();
    pool.commit_transaction(&TestTransaction::get_address(2), ReplayProtector::SequenceNumber(0));
...
fn test_include_gas_upgraded() {
    ...
    let low_gas_signed_txn = add_txn(
        &mut pool,
        TestTransaction::new(address_index, ReplayProtector::SequenceNumber(sequence_number), low_gas_price),
    )
    .unwrap();
    ...
    let high_gas_signed_txn = add_txn(
        &mut pool,
        TestTransaction::new(address_index, ReplayProtector::SequenceNumber(sequence_number), high_gas_price),
    )
    .unwrap();
